    ...
    screen.innerHTML = `
        <div class="row justify-content-center"><div class="col-lg-7">
            <div class="d-flex justify-content-between align-items-center mb-3">
                <small class="text-muted">💀 ${t('survivalMode')} #${survivalQuestionNumber + 1}</small>
                <div><span class="me-2">${heartsStr}</span><small class="text-muted">🏆 ${score}</small></div>
            </div>
            <div class="d-flex justify-content-between align-items-center mb-3">
                <small style="color: ${diffColor}">⚡ ${t('survivalDifficulty')}: ${survivalDifficultyLevel}/10</small>
                <small class="text-muted">🔥 ${t('survivalCombo')}: x${survivalCombo}</small>
            </div>
            <div class="d-flex justify-content-center mb-4">${typeof createCircularTimer === 'function' ? createCircularTimer(QUIZ_SETTINGS.timePerQuestion) : ''}</div>
            <div class="bg-card rounded-4 p-4 p-md-5 mb-4">
                <span class="badge bg-accent bg-opacity-25 text-accent rounded-pill px-3 py-2 mb-3">
                    ${typeof getCategoryName === 'function' ? getCategoryName(question.category) : question.category || t('anyCategory')} • ${typeof getDifficultyLabel === 'function' ? getDifficultyLabel(question.difficulty) : question.difficulty}
                </span>
        ...
        screen.innerHTML = `
            <div class="row justify-content-center"><div class="col-lg-6 text-center py-5">
                ...
                <div class="bg-card rounded-4 p-4 mb-4">
                    ...
                    <p class="text-muted">${t('survivalQuestionsAnswered')}: ${survivalQuestionNumber} | ${t('survivalBestCombo')}: x${survivalBestCombo}</p>

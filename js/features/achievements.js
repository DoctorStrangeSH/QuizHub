// ============================================
// QuizHub — Система достижений v3.0
// Исправленные условия + новые достижения
// ============================================
...
const ACHIEVEMENTS = [
  ...
  // ===== ВРЕМЯ СУТОК (ИСПРАВЛЕНО) =====
  { id: 'night_owl', name: 'Полуночник', desc: 'Пройти квиз с 00:00 до 06:00', icon: '🦉', 
    condition: () => { 
      const h = new Date().getHours(); 
      return h >= 0 && h < 6; 
    } 
  },
  { id: 'early_bird', name: 'Жаворонок', desc: 'Пройти квиз с 06:00 до 10:00', icon: '🌅', 
    condition: () => { 
      const h = new Date().getHours(); 
      return h >= 6 && h < 10; 
    } 
  },
  { id: 'afternoon', name: 'Дневной', desc: 'Пройти квиз с 12:00 до 16:00', icon: '☀️', 
    condition: () => { 
      const h = new Date().getHours(); 
      return h >= 12 && h < 16; 
    } 
  },
  { id: 'evening', name: 'Вечерний', desc: 'Пройти квиз с 18:00 до 22:00', icon: '🌆', 
    condition: () => { 
      const h = new Date().getHours(); 
      return h >= 18 && h < 22; 
    } 
...
function updateStats(result) {
  ...
  if (!quizStats.languagesUsed.includes(lang)) {
    quizStats.languagesUsed.push(lang);
  }
  ...
  if (!quizStats.difficultiesCompleted.includes(diff)) {
    quizStats.difficultiesCompleted.push(diff);
  }
  ...
  if (category !== 'any' && !quizStats.categoriesCompleted.includes(category)) {
    quizStats.categoriesCompleted.push(category);
  }
  ...
  // Режимы
  if (result.difficulty === 'survival') {
    quizStats.survivalPlayed = (quizStats.survivalPlayed || 0) + 1;
    if (typeof survivalQuestionNumber !== 'undefined' && survivalQuestionNumber > quizStats.survivalMaxQuestions) {
      quizStats.survivalMaxQuestions = survivalQuestionNumber;
    }
  }
...
function updateDayStreak() {
  ...
  if (quizStats.lastActiveDate === yesterday) {
    quizStats.dayStreak = (quizStats.dayStreak || 0) + 1;
  } else if (quizStats.lastActiveDate !== today) {
    quizStats.dayStreak = 1;
  }
  
  quizStats.lastActiveDate = today;
  
  if (quizStats.dayStreak >= 7) {
    const bonus = Math.min(quizStats.dayStreak * 5, 100);
    addXP(bonus);
  }
}
...
function checkAchievements(result) {
  // Обновляем социальную статистику
  ...
  const stats = {
    ...
    perfectQuizzes: quizStats.perfectQuizzes || 0,
    englishQuizzes: quizStats.englishQuizzes || 0,
    russianQuizzes: quizStats.russianQuizzes || 0,
    survivalPlayed: quizStats.survivalPlayed || 0,
    survivalMaxQuestions: quizStats.survivalMaxQuestions || 0,
    timedGames: quizStats.timedGames || 0,
    duelsPlayed: quizStats.duelsPlayed || 0,
    categoriesCompleted: (quizStats.categoriesCompleted || []).length,
    scienceQuizzes: quizStats.scienceQuizzes || 0,
    historyQuizzes: quizStats.historyQuizzes || 0,
    sportQuizzes: quizStats.sportQuizzes || 0,
    friendsCount: quizStats.friendsCount || 0,
    giftsSent: quizStats.giftsSent || 0,
    inTeam: quizStats.inTeam || false,
    closeWin: quizStats.closeWin || false,
  };
  ...
  ACHIEVEMENTS.forEach(ach => {
    if (!unlockedAchievements.includes(ach.id)) {
      try {
        ...
        console.error(`Ошибка проверки достижения ${ach.id}:`, e);
      }
    }
  });
      ...
      <div class="col-lg-6">
        ...
        <div class="bg-card rounded-4 p-4 mb-4">
          ...
          <div class="d-flex align-items-center gap-2 mb-2">
            ...
            <small class="text-muted">Ур. ${level.level}</small>
          </div>

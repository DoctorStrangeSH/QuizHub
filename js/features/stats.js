// ============================================
// QuizHub — Статистика и графики v2.1
// ============================================
    ...
    <div class="row justify-content-center">
      <div class="col-lg-8">
        ...
        <div class="row g-3 mb-4">
          ...
          <div class="col-6 col-md-3">
            <div class="bg-card rounded-4 p-3 text-center">
              <p class="display-6 fw-bold text-success mb-0">${dayStreak}</p>
        ...
        <div class="bg-card rounded-4 p-4 mb-4">
          ...
          <div class="row g-3">
            <div class="col-6">
              <p class="text-muted mb-1">${t('avgScore')}</p>
              <p class="fw-bold">${avgScore}</p>
            </div>
            <div class="col-6">
              <p class="text-muted mb-1">${t('languages')}</p>
              <p class="fw-bold">${(stats.languagesUsed || []).join(', ') || '—'}</p>
            </div>
            <div class="col-6">
              <p class="text-muted mb-1">${t('fastestAnswer')}</p>
              <p class="fw-bold">${stats.fastestAnswer < 999 ? stats.fastestAnswer + ' ' + t('seconds') : '—'}</p>
            </div>
            <div class="col-6">
              <p class="text-muted mb-1">${t('bestStreak')}</p>
              <p class="fw-bold">${stats.maxStreak || 0} ${t('correctShort')}</p>
            </div>
            <div class="col-6">
              <p class="text-muted mb-1">${t('totalXP')}</p>
              <p class="fw-bold">${totalXP} XP</p>
            </div>
            <div class="col-6">
              <p class="text-muted mb-1">${t('level')}</p>
              <p class="fw-bold">${typeof getLevelName === 'function' ? getLevelName(level) : t('lvl_novice')}</p>
            </div>
            <div class="col-6">
              <p class="text-muted mb-1">${t('fastestQuiz')}</p>
              <p class="fw-bold">${stats.fastestQuiz < 9999 ? formatTime(stats.fastestQuiz) : '—'}</p>
            </div>
            <div class="col-6">
              <p class="text-muted mb-1">${t('perfectQuizzes')}</p>
              <p class="fw-bold">${stats.perfectQuizzes || 0}</p>
            </div>
          </div>
        </div>
        ...
        <div class="text-center mt-4">
          ...
          <button class="btn btn-outline-accent rounded-pill px-4 ms-2" onclick="showScreen('achievements')">
            <i class="bi bi-award me-2"></i>${t('achievements')}
          </button>
        </div>
      </div>
    </div>
      ...
      options: {
        ...
        scales: {
          ...
          y: {
            ...
            beginAtZero: true,
            suggestedMax: Math.max(...last20, 10) + 20
          }
        }
      }
    });

...
async function startTimedMode() {
  ...
  QUIZ_SETTINGS.totalQuestions = 999; QUIZ_SETTINGS.timePerQuestion = 999; globalTimeLeft = 60;
  quizQuestions = await fetchQuestions('any', 'any', 100);
  if (quizQuestions.length === 0) { showScreen('home'); return; }
  currentQuestionIndex = 0; score = 0; maxStreak = 0;
...
function handleTimedAnswer(selectedIndex) {
  ...
  document.querySelectorAll('.btn-answer').forEach((b,i) => {
    ...
    if(i===selectedIndex&&!isCorrect){b.classList.add('wrong');if(typeof playWrongSound==='function')playWrongSound();}
  });
...
async function finishTimedMode() {
  ...
  const result = { playerName: document.getElementById('player-name')?.value?.trim() || 'Гость', score, totalTime: 60, correctAnswers: correctAnswersCount, difficulty: 'timed', category: 'На время', date: new Date().toISOString(), userId: typeof currentUser !== 'undefined' ? currentUser?.uid : null };

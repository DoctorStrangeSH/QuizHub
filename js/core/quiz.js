      ...
      <div class="col-lg-7">
        ...
        <div class="d-flex justify-content-between align-items-center mb-3">
          ...
          <div class="d-flex align-items-center gap-2">
            <div class="animation-indicator ${animationType === 'flip' ? 'active' : ''}" onclick="switchAnimation('flip')" title="Flip">🔄</div>
            <div class="animation-indicator ${animationType === 'slide' ? 'active' : ''}" onclick="switchAnimation('slide')" title="Слайд">⬆️</div>
            <div class="animation-indicator ${animationType === 'scale' ? 'active' : ''}" onclick="switchAnimation('scale')" title="Масштаб">💫</div>
...
function switchAnimation(type) {
  ...
  document.querySelectorAll('.animation-indicator').forEach(el => {
    el.classList.remove('active');
  });
  
  const types = ['flip', 'slide', 'scale'];
  const index = types.indexOf(type);
  const indicators = document.querySelectorAll('.animation-indicator');
  if (indicators[index]) {
...
function handleAnswer(selectedIndex) {
  ...
  buttons.forEach((btn, i) => {
    ...
    // Подсвечиваем неправильный выбор
  ...
  if (currentQuestionIndex < QUIZ_SETTINGS.totalQuestions) {
    ...
    if (card) {
      ...
      card.addEventListener('animationend', function handler() {
        ...
        // Анимация появления
        setTimeout(() => {
          const newCard = document.querySelector('.question-card-inner');
          if (newCard) {
...
async function finishQuiz() {
  ...
  // Событие: квиз завершён (для ai.js и других модулей)
  ...
  // Начисляем монеты
  if (typeof awardQuizCoins === 'function') {
    const coins = awardQuizCoins(result);
    if (coins > 0) {
      showToast(`+${coins} 🪙 заработано!`, 'success');
    }
  }
  
  // Автосохранение в Firestore
...
function renderResultScreen(result) {
  ...
  // Конфетти и фанфары при хорошем результате
  if (result.score >= 70) {
    ...
    if (typeof playFanfareSound === 'function') {
      playFanfareSound();
    }
  }
  ...
  // Кнопки шаринга
  if (typeof addShareButtons === 'function') {
...
function renderTimedModeScreen() {
  ...
  if (typeof addVoiceButton === 'function') {
...
function updateGlobalTimerDisplay() {
  ...
  if (circle) {
    circle.classList.toggle('timer-danger', globalTimeLeft <= 10);
  }
}

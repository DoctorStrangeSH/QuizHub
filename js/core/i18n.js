// ============================================
// QuizHub — Система переводов v2.2
// ============================================
...
function updateAllTranslations() {
  ...
  document.querySelectorAll('.btn-difficulty').forEach(btn => { const d = btn.dataset.difficulty; if (diffLabels[d]) btn.textContent = diffLabels[d]; });
  ...
  if (catSelect) {
    const catTrans = { any: t('anyCategory'), science: '🔬 '+t('science'), history: '📜 '+t('history'), geography: '🌍 '+t('geography'), sport: '⚽ '+t('sport'), cinema: '🎬 '+t('cinema'), art: '🎨 '+t('art'), music: '🎵 '+t('music'), it: '💻 '+t('it'), literature: '📚 '+t('literature'), food: '🍔 '+t('food'), animals: '🐾 '+t('animals'), space: '🚀 '+t('space') };
  ...
  document.querySelectorAll('.header-actions-collapsible .btn span').forEach(span => {
    ...
    const key = txt==='Лидеры'?'leaders':txt==='Ачивки'?'achievements':txt==='Статистика'?'stats':txt==='Друзья'?'friends':null;

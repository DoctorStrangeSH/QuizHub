...
async function joinTournament(type) {
    ...
    if (config.entryFee > 0) {
        ...
        if (typeof addCoins === 'function') addCoins(-config.entryFee);
    }
    ...
    try {
        ...
        if (typeof subscribeToTournament === 'function') {
        ...
        <div class="row justify-content-center">
            <div class="col-lg-6 text-center py-5">
                ...
                <button class="btn btn-accent rounded-pill px-4" onclick="showScreen('home')">
                    <i class="bi bi-house me-2"></i>${typeof t === 'function' ? t('home') : 'На главную'}
                </button>
            </div>
        </div>
    `;
...
function renderTournamentScreen() {
    ...
    screen.innerHTML = `
        <div class="row justify-content-center">
            <div class="col-lg-6 text-center py-5">
                <h2 class="fw-bold font-display mb-4">🏆 ${typeof t === 'function' ? t('tournamentTitle') : 'Турниры'}</h2>
                <div class="row g-3 mb-4">
                    <div class="col-md-4">
                        <div class="bg-card rounded-4 p-4">
                            <span class="fs-1">📅</span>
                            <h5 class="fw-bold mt-2">${typeof t === 'function' ? t('dailyTournament') : 'Ежедневный'}</h5>
                            <p class="text-accent fw-bold">🏆 100 ${typeof t === 'function' ? t('tournamentReward') : 'монет'}</p>
                            <p class="text-success small">${typeof t === 'function' ? t('free') : 'Бесплатно'}</p>
                            <button class="btn btn-accent rounded-pill px-4 mt-2 w-100" onclick="joinTournament('daily')">
                                ${typeof t === 'function' ? t('participate') : 'Участвовать'}
                            </button>
                        </div>
                    </div>
                    <div class="col-md-4">
                        <div class="bg-card rounded-4 p-4">
                            <span class="fs-1">🏆</span>
                            <h5 class="fw-bold mt-2">${typeof t === 'function' ? t('weeklyTournament') : 'Еженедельный'}</h5>
                            <p class="text-accent fw-bold">🏆 500 ${typeof t === 'function' ? t('tournamentReward') : 'монет'}</p>
                            <p class="text-muted small">${typeof t === 'function' ? t('entryFee') : 'Взнос'}: 50 🪙</p>
                    ...
                    <div class="col-md-4">
                        <div class="bg-card rounded-4 p-4">
                            <span class="fs-1">⚡</span>
                            <h5 class="fw-bold mt-2">${typeof t === 'function' ? t('blitzTournament') : 'Блиц-турнир'}</h5>
                            <p class="text-accent fw-bold">🏆 200 ${typeof t === 'function' ? t('tournamentReward') : 'монет'}</p>
                            <p class="text-muted small">${typeof t === 'function' ? t('entryFee') : 'Взнос'}: 25 🪙</p>

...
let wasOffline = false; // Флаг: был ли пользователь офлайн
let isFirstCheck = true; // Первая проверка при загрузке
...
function updateOnlineStatus() {
  ...
  // Показываем уведомления только при РЕАЛЬНОМ изменении статуса
  ...
  if (isOnline && !wasOnline && wasOffline) {
    // Только что подключились (и до этого были офлайн)
...
async function syncOfflineResults() {
  ...
  if (synced > 0) {
    // Успешно синхронизировали — не показываем Toast глобально
    console.log(`✅ Синхронизировано результатов: ${synced}`);
  }
